# model file: ../example-models/misc/moving-avg/arma11_alt.stan
import torch
import pyro
...
def model(data, params):
    ...
    err = _pyro_assign(err, ((_index_select(y, 1 - 1)  - mu) + (phi * mu)))
    err =  _pyro_sample(err, "err", "normal", [0, sigma])
    for t in range(2, to_int(T) + 1):

        err = _pyro_assign(err, (_index_select(y, t - 1)  - ((mu + (phi * _index_select(y, (t - 1) - 1) )) + (theta * err))))
        err =  _pyro_sample(err, "err", "normal", [0, sigma])
    # }

# model file: ../example-models/bugs_examples/vol3/data_cloning/seeds.stan
import torch
import pyro
...
def init_params(data, params):
    ...
    params["b"] = init_vector("b", dims=(I, K)) # vector
...
def model(data, params):
    ...
    for i in range(1, to_int(I) + 1):
        ...
        b[i - 1] =  _pyro_sample(_index_select(b, i - 1) , "b[%d]" % (to_int(i-1)), "normal", [0.0, sigma])
        n[i - 1] =  _pyro_sample(_index_select(n, i - 1) , "n[%d]" % (to_int(i-1)), "binomial_logit", [_index_select(N, i - 1) , _call_func("add", [(((alpha0 + (alpha1 * _index_select(x1, i - 1) )) + (alpha2 * _index_select(x2, i - 1) )) + (alpha12 * _index_select(x1x2, i - 1) )),_index_select(b, i - 1) ])], obs=_index_select(n, i - 1) )

# model file: ../example-models/BPA/Ch.09/tempemi.stan
import torch
import pyro
...
def validate_data_def(data):
    ...
    check_constraints(y, low=1, high=2, dims=[nind, n_occasions])
...
def init_params(data, params):
    ...
    params["mean_psiIO"] = init_real("mean_psiIO", low=0, high=1) # real/double
    params["mean_psiOI"] = init_real("mean_psiOI", low=0, high=1) # real/double
...
def model(data, params):
    ...
    mean_psiIO = params["mean_psiIO"]
    mean_psiOI = params["mean_psiOI"]
    mean_p = params["mean_p"]
    # initialize transformed parameters
    phi = init_vector("phi", low=0, high=1, dims=(n_occ_minus_1)) # vector
    psiIO = init_vector("psiIO", low=0, high=1, dims=(n_occ_minus_1)) # vector
    psiOI = init_vector("psiOI", low=0, high=1, dims=(n_occ_minus_1)) # vector
    ...
    for t in range(1, to_int(n_occ_minus_1) + 1):
        ...
        phi[t - 1] = _pyro_assign(phi[t - 1], mean_phi)
        psiIO[t - 1] = _pyro_assign(psiIO[t - 1], mean_psiIO)
        psiOI[t - 1] = _pyro_assign(psiOI[t - 1], mean_psiOI)
        p[t - 1] = _pyro_assign(p[t - 1], mean_p)
    for i in range(1, to_int(nind) + 1):

        for t in range(1, to_int(n_occ_minus_1) + 1):

            ps[1 - 1][i - 1][t - 1][1 - 1] = _pyro_assign(ps[1 - 1][i - 1][t - 1][1 - 1], (_index_select(phi, t - 1)  * (1.0 - _index_select(psiIO, t - 1) )))
            ps[1 - 1][i - 1][t - 1][2 - 1] = _pyro_assign(ps[1 - 1][i - 1][t - 1][2 - 1], (_index_select(phi, t - 1)  * _index_select(psiIO, t - 1) ))
            ps[1 - 1][i - 1][t - 1][3 - 1] = _pyro_assign(ps[1 - 1][i - 1][t - 1][3 - 1], (1.0 - _index_select(phi, t - 1) ))
            ps[2 - 1][i - 1][t - 1][1 - 1] = _pyro_assign(ps[2 - 1][i - 1][t - 1][1 - 1], (_index_select(phi, t - 1)  * _index_select(psiOI, t - 1) ))
            ps[2 - 1][i - 1][t - 1][2 - 1] = _pyro_assign(ps[2 - 1][i - 1][t - 1][2 - 1], (_index_select(phi, t - 1)  * (1.0 - _index_select(psiOI, t - 1) )))
            ps[2 - 1][i - 1][t - 1][3 - 1] = _pyro_assign(ps[2 - 1][i - 1][t - 1][3 - 1], (1.0 - _index_select(phi, t - 1) ))
            ...
            po[1 - 1][i - 1][t - 1][2 - 1] = _pyro_assign(po[1 - 1][i - 1][t - 1][2 - 1], (1.0 - _index_select(p, t - 1) ))
            po[2 - 1][i - 1][t - 1][1 - 1] = _pyro_assign(po[2 - 1][i - 1][t - 1][1 - 1], 0.0)
            po[2 - 1][i - 1][t - 1][2 - 1] = _pyro_assign(po[2 - 1][i - 1][t - 1][2 - 1], 1.0)

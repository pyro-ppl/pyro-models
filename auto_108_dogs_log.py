# model file: ../example-models/ARM/Ch.24/dogs_log.stan
import torch
import pyro
...
def model(data, params):
    ...
    for j in range(1, to_int(n_dogs) + 1):
        ...
        for t in range(1, to_int(n_trials) + 1):
            p[j - 1][t - 1] = _pyro_assign(p[j - 1][t - 1], _call_func("inv_logit", [((_index_select(beta, 1 - 1)  * _index_select(_index_select(n_avoid, j - 1) , t - 1) ) + (_index_select(beta, 2 - 1)  * _index_select(_index_select(n_shock, j - 1) , t - 1) ))]))
    # model block

    beta[1 - 1] =  _pyro_sample(_index_select(beta, 1 - 1) , "beta[%d]" % (to_int(1-1)), "uniform", [-(100), 0])
    beta[2 - 1] =  _pyro_sample(_index_select(beta, 2 - 1) , "beta[%d]" % (to_int(2-1)), "uniform", [0, 100])
    for i in range(1, to_int(n_dogs) + 1):

        for j in range(1, to_int(n_trials) + 1):
            y[i - 1][j - 1] =  _pyro_sample(_index_select(_index_select(y, i - 1) , j - 1) , "y[%d][%d]" % (to_int(i-1),to_int(j-1)), "bernoulli", [_index_select(_index_select(p, i - 1) , j - 1) ], obs=_index_select(_index_select(y, i - 1) , j - 1) )

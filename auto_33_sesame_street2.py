# model file: ../example-models/ARM/Ch.23/sesame_street2.stan
import torch
import pyro
...
def init_params(data, params):
    ...
    params["ag"] = init_matrix("ag", dims=(J, 2)) # matrix
    params["b"] = init_real("b") # real/double
    params["d"] = init_real("d") # real/double
    params["mu_ag"] = init_vector("mu_ag", dims=(2)) # vector
    params["phi_y"] = init_real("phi_y") # real/double
    params["phi_t"] = init_real("phi_t") # real/double
    ...
    params["sigma_py"] = init_real("sigma_py", low=0, high=100) # real/double
    params["sigma_pt"] = init_real("sigma_pt", low=0, high=100) # real/double
...
def model(data, params):
    ...
    Sigma_ag = init_matrix("Sigma_ag", low=0., dims=(2, 2)) # cov-matrix
    Sigma_yt = init_matrix("Sigma_yt", low=0., dims=(2, 2)) # cov-matrix
    yt_hat = init_matrix("yt_hat", dims=(N, 2)) # matrix
    ...
    for i in range(1, to_int(N) + 1):
        ...
        yt_hat[i - 1][1 - 1] = _pyro_assign(yt_hat[i - 1][1 - 1], ((_index_select(a, siteset[i - 1] - 1)  + ((b * d) * _index_select(z, i - 1) )) + (phi_y * _index_select(pretest, i - 1) )))
        yt_hat[i - 1][2 - 1] = _pyro_assign(yt_hat[i - 1][2 - 1], ((_index_select(g, siteset[i - 1] - 1)  + (d * _index_select(z, i - 1) )) + (phi_t * _index_select(pretest, i - 1) )))
    # model block
    ...
    d =  _pyro_sample(d, "d", "normal", [0, 100])
    b =  _pyro_sample(b, "b", "normal", [0, 100])
    phi_y =  _pyro_sample(phi_y, "phi_y", "normal", [0, sigma_py])
    phi_y =  _pyro_sample(phi_y, "phi_y", "normal", [0, sigma_pt])
    ...
    mu_ag[1 - 1] =  _pyro_sample(_index_select(mu_ag, 1 - 1) , "mu_ag[%d]" % (to_int(1-1)), "normal", [0, 100])
    mu_ag[2 - 1] =  _pyro_sample(_index_select(mu_ag, 2 - 1) , "mu_ag[%d]" % (to_int(2-1)), "normal", [0, 100])
    for i in range(1, to_int(N) + 1):
        _call_func("transpose", [yt[i - 1]]) =  _pyro_sample(_call_func("transpose", [_index_select(yt, i - 1) ]), "_call_func( transpose , [yt[i - 1]])", "multi_normal", [_call_func("transpose", [_index_select(yt_hat, i - 1) ]), Sigma_yt])
    for j in range(1, to_int(J) + 1):
        _call_func("transpose", [ag[j - 1]]) =  _pyro_sample(_call_func("transpose", [_index_select(ag, j - 1) ]), "_call_func( transpose , [ag[j - 1]])", "multi_normal", [mu_ag, Sigma_ag])

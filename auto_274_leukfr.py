# model file: ../example-models/bugs_examples/vol1/leukfr/leukfr.stan
import torch
import pyro
...
def validate_data_def(data):
    ...
    assert 'Npair' in data, 'variable not found in data: key=Npair'
    ...
    check_constraints(Npair, low=0, dims=[1])
    check_constraints(pair, low=0, dims=[N])
...
def init_params(data, params):
    ...
    params["b"] = init_real("b", dims=(Npair)) # real/double
...
def model(data, params):
    ...
    b =  _pyro_sample(b, "b", "normal", [0, sigma])
    for j in range(1, to_int(NT) + 1):
        ...
        for i in range(1, to_int(N) + 1):
            ...
            if (as_bool(_call_func("logical_neq", [_index_select(_index_select(Y, i - 1) , j - 1) ,0]))):
                pyro.sample("_call_func( poisson_log , [_index_select(_index_select(dN, i - 1) , j - 1) ,((_index_select(_index_select(Y, i - 1) , j - 1)  * _call_func( exp , [((beta * _index_select(Z, i - 1) ) + _index_select(b, pair[i - 1] - 1) )])) * _index_select(dL0, j - 1) )])[%d][%d]" % (i, j), dist.Bernoulli(_call_func("poisson_log", [_index_select(_index_select(dN, i - 1) , j - 1) ,((_index_select(_index_select(Y, i - 1) , j - 1)  * _call_func("exp", [((beta * _index_select(Z, i - 1) ) + _index_select(b, pair[i - 1] - 1) )])) * _index_select(dL0, j - 1) )])), obs=(1));

# model file: ../example-models/BPA/Ch.07/cjs_t_t.stan
import torch
import pyro
...
def init_params(data, params):
    ...
    params["phi_t"] = init_vector("phi_t", low=0, high=1, dims=(n_occ_minus_1)) # vector
    params["p_t"] = init_vector("p_t", low=0, high=1, dims=(n_occ_minus_1)) # vector
...
def model(data, params):
    ...
    for i in range(1, to_int(nind) + 1):
        ...
        for t in range(to_int(first[i - 1]), to_int(n_occ_minus_1) + 1):
            ...
            phi[i - 1][t - 1] = _pyro_assign(phi[i - 1][t - 1], _index_select(phi_t, t - 1) )
            p[i - 1][t - 1] = _pyro_assign(p[i - 1][t - 1], _index_select(p_t, t - 1) )

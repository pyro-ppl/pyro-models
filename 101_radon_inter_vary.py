...
def model(data, params):
    ...
    beta =  pyro.sample("beta", dist.Normal(100 * mu_beta, sigma_beta).expand([2]))
    mu_a =  pyro.sample("mu_a", dist.Normal(0., 1.))
    a =  pyro.sample("a", dist.Normal(mu_a, sigma_a).expand([85]))
    mu_b =  pyro.sample("mu_b", dist.Normal(0., 1.))
    b =  pyro.sample("b", dist.Normal((0.1 * mu_b), sigma_b).expand([85]))

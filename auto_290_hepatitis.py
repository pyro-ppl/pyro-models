# model file: ../example-models/bugs_examples/vol3/hepatitis/hepatitis.stan
import torch
import pyro
...
def model(data, params):
    ...
    sigma_y = init_real("sigma_y") # real/double
    sigma_alpha = init_real("sigma_alpha") # real/double
    sigma_beta = init_real("sigma_beta") # real/double
    ...
    for n in range(1, to_int(N1) + 1):
        ...
        m[n - 1] = _pyro_assign(m[n - 1], ((_index_select(alpha, oldn - 1)  + (_index_select(beta, oldn - 1)  * (_index_select(tvec1, n - 1)  - 6.5))) + (gamma * (_index_select(y0, oldn - 1)  - y0_mean))))

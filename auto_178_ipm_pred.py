# model file: ../example-models/BPA/Ch.11/ipm_pred.stan
import torch
import pyro
...
def validate_data_def(data):
    ...
    assert 't_pred' in data, 'variable not found in data: key=t_pred'
    # initialize data
    nyears = data["nyears"]
    y = data["y"]
    J = data["J"]
    ...
    check_constraints(t_pred, low=1, dims=[1])
...
def model(data, params):
    ...
    sjuv = init_vector("sjuv", low=0, high=1, dims=(nyears)) # vector
    sad = init_vector("sad", low=0, high=1, dims=(nyears)) # vector
    p = init_vector("p", low=0, high=1, dims=(nyears)) # vector
    f = init_vector("f", low=0, dims=(nyears)) # vector
    ...
    for t in range(1, to_int(nyears) + 1):

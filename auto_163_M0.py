# model file: ../example-models/BPA/Ch.06/M0.stan
import torch
import pyro
...
def model(data, params):
    ...
    for i in range(1, to_int(M) + 1):
        if (as_bool(_call_func("logical_gt", [_index_select(s, i - 1) ,0]))):
            pyro.sample("(_call_func( bernoulli_log , [1,omega]) + _call_func( binomial_log , [_index_select(s, i - 1) ,T,p]))[%d]" % (i), dist.Bernoulli((_call_func("bernoulli_log", [1,omega]) + _call_func("binomial_log", [_index_select(s, i - 1) ,T,p]))), obs=(1));
        else: 
            pyro.sample("_call_func( log_sum_exp , [(_call_func( bernoulli_log , [1,omega]) + _call_func( binomial_log , [0,T,p])),_call_func( bernoulli_log , [0,omega])])[%d]" % (i), dist.Bernoulli(_call_func("log_sum_exp", [(_call_func("bernoulli_log", [1,omega]) + _call_func("binomial_log", [0,T,p])),_call_func("bernoulli_log", [0,omega])])), obs=(1));

# model file: ../example-models/BPA/Ch.07/cjs_ind_raneff.stan
import torch
import pyro
...
def init_params(data, params):
    ...
    params["epsilon"] = init_vector("epsilon", dims=(nind)) # vector
...
def model(data, params):
    ...
    for i in range(1, to_int(nind) + 1):
        ...
        for t in range(to_int(first[i - 1]), to_int(n_occ_minus_1) + 1):
            ...
            phi[i - 1][t - 1] = _pyro_assign(phi[i - 1][t - 1], _call_func("inv_logit", [(mu + _index_select(epsilon, i - 1) )]))
